...
const ShowPosts = props => {
  ...
  useEffect(() => {
    ...
    db.collection('Posts')
      .where(
        'groupId',
        'in',
        props?.user?.subscribedIds?.length > 0
          ? props?.user?.subscribedIds
          : ['abc'],
      )
    ...
    db.collection('States')
      ?.where('country', '==', props?.user?.country?.toLowerCase())
      .onSnapshot(documentSnapshot => {
        getStates();
      });
    if (props?.updateApp) setUpdateMod(true);
    else getDataPop();
  }, []);
  ...
  async function onShare(item, index) {
    copyToClip(
      'whatsapp://send?text=https://mocooproject.page.link/posts/${item?.id}',
    );
    hideMenu(item, index);
  }
  ...
  return (
    <SafeAreaView style={{...GlobalStyles.container}}>
      ...
      <ScrollView contentContainerStyle={{paddingBottom: HP(5)}}>
        <View
          ...
          <CustomBtn1
            ...
            txt={'Posts'}
            style={{
              ...GlobalStyles.btnView,
              backgroundColor:
                optTab != 'Posts' ? palette.gray : palette.airbnb,
            }}
          />
          <CustomBtn1
            onPress={() => {
              setOptTab('Subscribed');
            }}
            txt={'Subscribed'}
            style={{
              ...GlobalStyles.btnView,
              backgroundColor:
                optTab != 'Subscribed' ? palette.gray : palette.airbnb,
            }}
          />
        </View>
        {optTab == 'Posts' ? (
              ...
              renderItem={({item, index}) => {
                return (
                  <View
                    ...
                    <TouchableOpacity
                      onPress={() => {
                        props?.navigation?.navigate('ShowCities', item);
                      }}
                    ...
                    <TouchableOpacity
                      ...
                      />
                      <Text
                        style={{...GlobalStyles?.boldTxt, paddingLeft: WP(2)}}>
      ...
      <UpdateModal
        mod={updeteMod}
        onPress={() => {
          props?.updateAppClose
            ? AlertService.show('Update First')
            : setUpdateMod(false);
        }}
        onUpate={async () => {
          await Linking?.openURL(
            'https://play.google.com/store/apps/details?id=com.mocooproject',
          );
        }}
      />
    </SafeAreaView>
