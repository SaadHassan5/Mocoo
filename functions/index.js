...
exports.scheduledFunctionDelOntheGoFriendsPosts = functions.pubsub.schedule('every 5 minutes').onRun(async (context) => {
    ...
    for (let index = 0; index < res?.length; index++) {
        ...
        if (da?.setHours(da?.getHours() + 3) < new Date()?.getTime()) {
            console.log('Valid for delete', res[index]);
            await deleteData('FriendsPosts', res[index]?.postId)
        }
        else {
            console.log('Not valid', da?.setHours(da?.getHours() + 3), '===>', new Date().getTime());
        }
...
exports.scheduledFunctionDelStatus = functions.pubsub.schedule('every 5 minutes').onRun(async (context) => {
    const res = await filterCollections('Users', 'statusActive', '==', true)
    console.log('reeeees', res);
    for (let index = 0; index < res?.length; index++) {
...
exports.sendFriendStatusNotification = functions.firestore
    .document('Users/{id}')
    .onWrite(async (change, context) => {
        try {
            let after = change.after.data();
            ...
            if (after?.userStatus != before?.userStatus && after?.userStatus!="") {
                ...
                const payload = {
                    notification: {
                        title: `${after?.name?.split(' ')[0]} updated his status`,
                        body: `${after?.userStatus?.toUpperCase()}`,
                        sound: 'default',
                    },
                    data: { screen: 'friendStatus', friendId: after?.email }
                };
                const options = {
                    priority: 'high',
                    timeToLive: 60 * 60 * 24,
                ...
                allUsers?.map(i => {
                    if (i?.token) {
                        admin
                            ...
                            .sendToDevice(i?.token, payload, options)
                            .then(reponse => {
                                console.log('Send friend status Notification to Everybody ');
                            });
                    }
                })
            }
        } catch (error) {
            console.log('error in friend status Notification', error);
        }
    })